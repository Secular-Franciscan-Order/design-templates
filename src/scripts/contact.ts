...
const successHeading = success.querySelector<HTMLHeadingElement>("h3")!;
...
function challengeStatus(message: string, error = false) {
  if (!successVisible && !pending && !submissionFeedback) setStatus(message, error);
}
...
function resetChallenge() {
  ...
  waitForChallenge();
  try {
    if (!widget || !window.turnstile) throw new Error("Spam check unavailable");
    window.turnstile.reset(widget);
  } catch { challengeFailed("The spam check could not start. Please refresh the page or email bill@endian.dev."); }
}
function focusWithoutJump(element: HTMLElement) {
  element.focus({ preventScroll: true });
  const bounds = element.getBoundingClientRect();
  if (bounds.top < 0 || bounds.bottom > window.innerHeight) element.scrollIntoView({ block: "nearest" });
}
function showSuccess(fields: Record<string, string>) {
  successVisible = true;
  clearChallengeWait();
  token = "";
  button.disabled = true;
  for (const name of ["name", "email", "fraternity", "design"]) {
    (form.elements.namedItem(name) as HTMLInputElement | HTMLSelectElement).value = fields[name];
  }
  for (const name of ["message", "website"]) {
    (form.elements.namedItem(name) as HTMLInputElement | HTMLTextAreaElement).value = "";
  }
  success.querySelector<HTMLElement>("[data-submitted-email]")!.textContent = fields.email;
  setStatus("");
  form.hidden = true;
  success.hidden = false;
  focusWithoutJump(successHeading);
}
success.querySelector<HTMLButtonElement>("[data-send-another]")!.addEventListener("click", () => {
  ...
  form.hidden = false;
  resetChallenge();
  focusWithoutJump(form.querySelector<HTMLInputElement>("#contact-name")!);
});
form.addEventListener("submit", async (event) => {
  event.preventDefault();
  if (successVisible || pending || !form.reportValidity()) return;
  ...
  try {
    ...
    if (!successVisible) resetChallenge();
  }
});
if (siteKey) {
  ...
  script.addEventListener("load", () => {
    ...
    try {
      ...
      widget = turnstile.render(form.querySelector<HTMLElement>("[data-turnstile]")!, {
        ...
        callback: (value: string) => { if (successVisible) return; clearChallengeWait(); token = value; button.disabled = pending; challengeStatus(""); },
        "before-interactive-callback": () => { if (successVisible) return; clearChallengeWait(); challengeStatus("Please complete the spam check to send your message."); },
        "after-interactive-callback": () => { if (successVisible) return; if (!token) { challengeStatus(""); waitForChallenge(); } },
        "expired-callback": () => { if (successVisible) return; clearChallengeWait(); token = ""; button.disabled = true; challengeStatus("The spam check expired. Please complete it again."); },
